...
window.jscarousel = function (CarouselContainer, config) {
  function navigateToNextItem() {
    var transform = 'translateX(-' + currentPage * containerWidth + 'px)';
  ...
  function swipeMove(ev) {
    if (swipeStartXPosition !== null) {
      var translateX = 'translateX(-' + (currentPage * containerWidth + (swipeStartXPosition - resolveMouseX(ev))) + 'px)';
  ...
  function addPassiveListener(target, event, handler) {
    ...
    try {
      ...
      console.log('%c' + new Date().format('[%f %D, %y][%H:%N:%S %a]') + ':', 'background: #222; color: #bada55', 'addPassiveListener failed:', err);
    }
  ...
  while (CarouselContainer.children.length) {
    CarouselContainer.children[0].style = 'width: 100%; vertical-align: top; display: inline-block; white-space: pre-line;';
    ItemsWrapper.appendChild(CarouselContainer.children[0]);

    // might as well create pagination elements now
    var Page = document.createElement('div');
    Page.style = 'display: inline-block; width: 7px; height: 7px; background-color: #1a84a8; border-radius: 50%; margin-right: 10px; cursor: pointer;';
  ...
  PagesContainer.style = 'position: absolute; bottom: 20px; left: 50%; transform: translate(-50%, 0);';
